:- 
%%%%%%%%%%%%%%%%%%      BOSQUES     %%%%%%%%%%%%%%%%%%
%writeln('consultando...'),
consult('bosques/bioclimaBosques.pl'),
consult('bosques/biogeografiaBosques.pl'),
consult('bosques/factoresDiagnosticoBosques.pl'),
consult('bosques/fenologiaBosques.pl'),
consult('bosques/mesorelieveBosques.pl'),
consult('bosques/nombreBosques.pl'),
consult('bosques/pisoBioclimaticoBosques.pl'),
consult('bosques/reconocerBosques.pl'),
%%%%%%%%%%%%%%%%%%      BOSQUES     %%%%%%%%%%%%%%%%%%

%%%%%%%%%%%%%%%%%%      NIVELES     %%%%%%%%%%%%%%%%%%
consult('niveles/clasificadores.pl'),
consult('niveles/nivel1.pl'),
consult('niveles/nivel3.pl'),
consult('niveles/nivel4.pl'),
consult('niveles/nivel5.pl'),
consult('niveles/nivel6.pl'),
%%%%%%%%%%%%%%%%%%      NIVELES     %%%%%%%%%%%%%%%%%%

%%%%%%%%%%%%%%%%%%      DIVISIONES BIOGEOGRAFICAS     %%%%%%%%%%%%%%%%%%
consult('divisionesBiogeograficas/divisionesBiogeograficas.pl').
%%%%%%%%%%%%%%%%%%      DIVISIONES BIOGEOGRAFICAS     %%%%%%%%%%%%%%%%%%